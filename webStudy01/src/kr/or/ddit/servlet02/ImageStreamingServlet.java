...
//		String contentsPath = config.getInitParameter("contentsPath");
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String alreadyImages = utils.getCookieValues(Constants.IMAGESTREAMINGCOOKIENAME);
		String[] alreadyArray = null;
		if(StringUtils.isNotBlank(alreadyImages)) {
			alreadyArray = mapper.readValue(alreadyImages, String[])
		}
		...
		if(cookies != null) {
			for(Cookie tmp : cookies) {
				if("imageCookie".equals(tmp.getName())) {
					alreadyImages = URLDecoder.decode(tmp.getValue(),"UTF-8");
		...
		Cookie imageCookie = new Cookie("imageCookie", json);
		imageCookie.setPath(req.getContextPath());
		imageCookie.setMaxAge(60*60*24*2);
